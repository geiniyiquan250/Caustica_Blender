                                   ...
                                   const ccl_private RNGState *rng_state)
{
   ...
   * point one bounce back, that is the same caustic the photon map carries,
   * so it belongs to one of the two, not both. Only materials the classifier
   * calls casters qualify, and those are specular by definition - there is no
   * meaningful diffuse direct light to lose here. */
    ...
    const ccl_private RNGState *rng_state)
{
   ...
   * the one place the gather deposits light - hence diffuse_bounce == 1. A
   * second diffuse bounce ends the chain: from there the light is carried by
   * path tracing alone and taking it away would darken the image with nothing
   * to put back. A specular surface the photon map does not cast from ends it
   * as well. */
...

{
  ...
  if (!(sd.flag & SD_HAS_ONLY_VOLUME)) {
     ...
     * generation. Skipped after the first diffuse bounce: the map stores
     * only flux that arrived via specular chains, and only the direct (and
     * specularly seen) caustics are what path tracing cannot find.
     * Subsurface EXIT points qualify as well (the local path_flag still
     * carries PATH_RAY_SUBSURFACE here): lighting for SSS happens at the
     * exit, so the photon light must too - otherwise skin pixels whose
     * walks exit into a caustic-lit region (a floaty's submerged shell)
     * lose it and show a dark band one SSS radius above the waterline.
     * The exit hitpoint overwrites the entry hit of the same sample.
     * The geometric normal goes along for the gather plane test: bump/normal
     * maps tilt sd.N away from the true surface plane the photons were
     * deposited on, which would wrongly reject the whole neighborhood. */
    ...
    {
      /* The writer test up front (the callee re-checks it) spares every
       * non-writer sample the diffuse-albedo closure loop below - on GPU
       * that is N-1 of N samples per work computing a value the callee was
       * going to drop. Behavior-identical: the write was skipped anyway. */
      ...
      if (!is_zero(diffuse_albedo)) {
        const Spectrum weight = INTEGRATOR_STATE(state, path, throughput) * diffuse_albedo *
                                M_1_PI_F;
        /* NOTE: sd.Ng at SSS exit points is already outward facing (exit
         * lighting happens from outside); flipping it by SD_BACKFACING was
