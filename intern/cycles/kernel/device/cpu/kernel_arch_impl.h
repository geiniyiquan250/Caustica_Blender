...
void KERNEL_FUNCTION_FULL_NAME(film_photon_gather)(const ThreadKernelGlobalsCPU *kg,
                                                   ccl_global float *render_buffer,
                                                   const int x,
                                                   const int y,
                                                   const int offset,
...
{
#ifdef KERNEL_STUB
  STUB_ASSERT(KERNEL_ARCH, film_photon_gather);
  (void)kg;
  (void)render_buffer;
  (void)x;
  (void)y;
  ...
  (void)consume;
#else
  film_photon_gather_pixel(kg, render_buffer, x, y, offset, stride, num_samples, consume);
#endif
}

void KERNEL_FUNCTION_FULL_NAME(film_photon_smooth)(const ThreadKernelGlobalsCPU *kg,
                                                   ccl_global float *render_buffer,
                                                   const int x,
                                                   const int y,
                                                   const int sx,
...
{
#ifdef KERNEL_STUB
  STUB_ASSERT(KERNEL_ARCH, film_photon_smooth);
  (void)kg;
  (void)render_buffer;
  (void)x;
  (void)y;
