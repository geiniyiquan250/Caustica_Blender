...
/* Per-light-group caustics (CyclesPlus). The gather splits the flux into one
 * accumulator per group during the grid traversal, so that array is local to
 * the film kernel and needs a compile-time size. This is a CHUNK size, not a
 * limit: a scene with more groups is gathered in several passes over the grid
 * (8 groups each) instead of dropping any. Deliberately not a hard cap - a
 * capped group would render as a black pass, which looks like a bug and is
 * exactly the kind of silent truncation this project does not ship. */
...
enum PathRayFlag : uint32_t {
  ...
  /* Photon caustics (CyclesPlus): this path is the work's designated SPPM
   ...
   * the photon map already carries, so path tracing must not add it a second
   * time. Any diffuse bounce clears it. */
...
enum PassType {
  ...
  /* Photon caustics (CyclesPlus). The caustic contribution is delta-written
   * into the combined pass only, so without this pass the light passes no
   * longer add up to Combined - the caustic is the missing remainder. With
   * it the decomposition is complete again, and compositing can grade the
   * caustics on their own. */
  PASS_CAUSTICS,
   ...
   * WEIGHT:   throughput * diffuse_albedo / pi + octahedral-packed normal.
   * TAU:      accumulated (rescaled) flux tau rgb + photon count N.
   * STATE:    gather radius^2 + last combined sum written (rgb). */
  PASS_PHOTON_HITPOINT,
  PASS_PHOTON_WEIGHT,
  PASS_PHOTON_TAU,
  PASS_PHOTON_STATE,
  /* TAU_GROUP: one per light group, 3 components, the group's share of TAU.
   * Registered once per group and kept contiguous, so the gather can index it
...
#undef DECLARE_FILM_CONVERT_KERNEL
  ...
  /* Photon caustics display smoothing (CyclesPlus): maturity-faded
   * cross-bilateral filter over the raw SPPM estimates; display-only, the
   * per-pixel statistics stay untouched. */
  DEVICE_KERNEL_FILM_PHOTON_SMOOTH,

  /* Photon caustics tracing, one thread = one photon (CyclesPlus). */
  DEVICE_KERNEL_PHOTON_TRACE,

  /* Photon caustics device-resident binning (CyclesPlus): count deposits per
   * hash cell, initialize the per-cell scatter cursors from the prefix table,
   * scatter deposits into the cell-sorted arrays. Deposits never leave the
   * GPU; only the cell table crosses PCIe. */
