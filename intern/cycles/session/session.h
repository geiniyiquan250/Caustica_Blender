...
class Session {
 ...
 protected:
   ...
   * the next photon map restart invalidates its cached HDRI virtual suns. */
  unique_ptr<PhotonMap> photon_map_;
  bool photon_world_changed_ = true;
  /* Extra viewport samples scheduled beyond the configured count to let the
   ...
   * because a final render runs (see the park block in
