...
void Film::device_update(Device *device, DeviceScene *dscene, Scene *scene)
{
  ...
  kfilm->pass_photon_tau = PASS_UNUSED;
  kfilm->pass_photon_state = PASS_UNUSED;
  ...
  for (size_t i = 0; i < scene->passes.size(); i++) {
    ...
    if (!pass->get_lightgroup().empty()) {
      /* Photon caustics (CyclesPlus): the per-group caustics passes share this
       * sort bucket with Cycles' own Combined_<group> (both 3 components, both
       * lightgroup) and land behind them, because PASS_COMBINED sorts before
       * PASS_CAUSTICS and the sort is stable. They still get their OWN base
    ...
    switch (pass->get_type()) {
      ...
      case PASS_PHOTON_HITPOINT:
        kfilm->pass_photon_hitpoint = kfilm->pass_stride;
        break;
      case PASS_PHOTON_WEIGHT:
        kfilm->pass_photon_weight = kfilm->pass_stride;
        break;
      case PASS_PHOTON_TAU:
        kfilm->pass_photon_tau = kfilm->pass_stride;
        break;
      case PASS_PHOTON_STATE:
        kfilm->pass_photon_state = kfilm->pass_stride;
        break;
      case PASS_PHOTON_TAU_GROUP:
        /* One per group, contiguous - only the first offset is needed. */
        if (!have_photon_tau_group) {
          kfilm->pass_photon_tau_group = kfilm->pass_stride;
...
void Film::update_passes(Scene *scene)
{
  ...
  if (integrator->get_use_photon_caustics()) {
    ...
    add_auto_pass(scene, PASS_PHOTON_WEIGHT);
    add_auto_pass(scene, PASS_PHOTON_TAU);
    add_auto_pass(scene, PASS_PHOTON_STATE);

    /* One tau accumulator per caustics light group. Added in the order the
     * groups appear in the pass list - the same order update_lightgroups
     * numbers them in - so one index addresses both the tau block and the
     * Caustics_<group> block. They carry the group name only to keep
     * finalize_passes from merging them into one (it merges by type and name,
     * not by lightgroup); they are never registered with Blender, so they stay
     * internal exactly like the four passes above. */
    const vector<Pass *> passes_before_tau_groups = scene->passes;
    for (const Pass *pass : passes_before_tau_groups) {
      if (pass->get_type() == PASS_CAUSTICS && !pass->get_lightgroup().empty()) {
        const string name = string_printf("PhotonTauGroup_%s",
                                          pass->get_lightgroup().c_str());
        add_auto_pass(scene, PASS_PHOTON_TAU_GROUP, name.c_str());
      }
    }
  }
  ...
  for (const Pass *pass : passes_immutable) {
     ...
     * deliberately skips denoised passes ("we only store offsets of the
     * noisy passes"). Cycles' own denoised passes get that twin from
     * Blender, which registers "Noisy Image" and "Noisy Shadow Catcher" as
     * separate passes; Caustics has no such counterpart, so it must add its
     * own. Without it BOTH ends go quiet - nothing to write into, nothing to
     * read from - and the pass renders pure black. Kept unnamed so it stays
     * internal and does not show up as a second entry in the UI. */
...
void Film::finalize_passes(Scene *scene, const bool use_denoise)
{
  ...
  for (size_t i = 0; i < scene->passes.size(); i++) {
    ...
    for (Pass *new_pass : new_passes) {
       ...
       * come first: the noisy twin that the denoised Caustics pass depends on
       * would be swallowed by Caustics_<group>, leaving the visible Caustics
       * pass with nothing to read - the same black pass that took three
       * attempts to diagnose the first time. */
