...
void Integrator::device_update(Device *device, DeviceScene *dscene, Scene *scene)
{
   ...
   * made in the kernel, see svm/photon_caustics.h. */
   ...
   * them - see the reasoning in kernel/svm/photon_caustics.h. Replacing them
   * blacked out interiors, because Cycles' suppression takes the whole
   * specular closure with it and not just the caustic. Support hatch only. */
  static const bool replace_pt = getenv("CYCLESPLUS_PHOTON_REPLACE_PT") != nullptr;
  kintegrator->photon_replace_pt = replace_pt ? 1 : 0;

  /* Experiment (branch mis-caustic-partition): let the two estimators divide
   * the work instead of both reporting the same light. Measured overlap
   * without it: up to 1.5x too bright where Cycles finds the caustic too. */
  static const int partition_pt = []() {
    /* Bit 0: light reaching a path that runs into a lamp, the world or an
     * emitter over a specular chain the photon map carries. On by default.
     *
     * Bit 1: the direct light a ROUGH caster samples for itself. OFF, and it
     * must stay off until it is rebuilt per closure. Skipping the whole
     * direct light computation at a caster is too blunt: it also drops what
     * the surface's other closures contribute, and the photon map has
     * nothing to put in their place. Measured on the tester's bathroom
     * scene: 2% of the lit pixels fell to less than half their brightness,
     * and the same gate cost 45% of the lit pixels in the ocean rebuild
     * before scattering media were excluded. What it bought was 0.4% on a
     * frosted-glass sphere and nothing at all on the tester's cube - the
     * emission gate alone takes that case from +2.8% to +0.45% of ground
     * truth. Not worth a single dark pixel. */
    const char *env = getenv("CYCLESPLUS_PHOTON_PARTITION");
