  ...
  if (context.need_denoising()) {
    ...
    const std::array<PassType, 4> passes = {
        {/* Passes which will use real albedo when it is available. */
         ...
         /* Photon caustics (CyclesPlus): the caustic already carries the
          * receiving surface's albedo, so it denoises like combined. Costs
          * nothing when the pass is not enabled. */
         PASS_CAUSTICS,
