...
class PathTrace {
 public:
   ...
   * all render buffers after the next render work. */
  void set_photon_grid(const struct PhotonGrid *grid);

  /* Photon caustics: true while an uploaded photon generation still awaits
   * its consuming gather (which runs after the next render work). The
   * session uses this to schedule idle gather-only works. */
  bool photon_gather_pending() const
  {
