...
void PathTrace::render_pipeline(RenderWork render_work)
{
   ...
   * the scheduler grow works to hundreds of samples would starve the
   * refinement. */
  if (device_scene_->data.integrator.use_photon_caustics) {
    /* CYCLESPLUS_PHOTON_WORK_CAP: debug override for the work size. Cap 1
     * serializes the per-pixel samples between gathers, which rules the
     * same-pixel hitpoint-write race in or out (GPU fade bisection). */
    static const uint photon_limit = []() {
      const char *env = getenv("CYCLESPLUS_PHOTON_WORK_CAP");
      const int v = env ? atoi(env) : 0;
      return (v > 0) ? (uint)v : 16u;
    }();
   ...
   * resets it - skipping the call would let a stale cap stick: stock
   * Cycles' volume rhythm resets at every power of two, and switching the
   * photon checkbox off must return the session to stock work sizes. */
  render_scheduler_.set_limit_samples_per_update(limit_samples);
...
void PathTrace::path_trace(RenderWork &render_work)
{
   ...
   * launches; see data_template.h photon_writer_sample). */
  if (device_scene_->data.integrator.use_photon_caustics) {
    const int writer_sample = render_work.path_trace.start_sample +
                              render_work.path_trace.num_samples - 1;
    if (device_scene_->data.integrator.photon_writer_sample != writer_sample) {
      device_scene_->data.integrator.photon_writer_sample = writer_sample;
...
void PathTrace::set_photon_grid(const struct PhotonGrid *grid)
{
   ...
   * between render works only; the generation check skips redundant uploads
   * when no new batch finished since the last call. The SPPM gather that
   * consumes the batch runs after the next render work
   * (photon_gather_after_work). */
  const uint64_t generation = (grid && grid->num_photons > 0) ? grid->generation : 0;
  ...
  DeviceScene *dscene = device_scene_;
  KernelIntegrator *kintegrator = &dscene->data.integrator;

  /* CyclesPlus: the classifier's per-shader verdict, for the path tracer's
   * side of the caustic partition. One byte per shader - a few hundred over
   * PCIe next to megabytes of deposits. Without it the kernel cannot tell a
   * caster's specular bounce from a glossy tabletop's, and gating the wrong
   * one takes away light nothing replaces. */
  if (generation != 0 && grid->shader_caster != nullptr && grid->num_shader_caster > 0) {
    const size_t num_shaders = (size_t)grid->num_shader_caster;
    uint *caster_dst = dscene->photon_shader_caster.alloc(num_shaders);
    for (size_t i = 0; i < num_shaders; i++) {
      caster_dst[i] = (uint)grid->shader_caster[i];
    }
    dscene->photon_shader_caster.copy_to_device();
    kintegrator->photon_shader_count = (int)num_shaders;
  }
  else {
    kintegrator->photon_shader_count = 0;
  }
  ...
  if (generation == 0) {
    ...
    kintegrator->photon_batch_steady = 1;
  }
  else if (grid->device_resident) {
    /* Device-resident generation (GPU tracer + GPU binning): the deposits
     * never left VRAM. Allocate the target arrays, upload only the cell
     * prefix table and let the PhotonMap scatter GPU-side. */
    const int num = grid->num_photons;
    const int table = (int)grid->table_size;

    /* MEM_GLOBAL arrays only materialize through copy_to_device (global
     * alloc + bind); mem_zero has no global path - zero_to_device left the
     * device_pointer null and the scatter crashed. Allocate with headroom
    ...
    if (scattered) {
      ...
      kintegrator->photon_table_size = table;
      kintegrator->photon_radius = grid->radius;
      ...
      /* CYCLESPLUS_PHOTON_BIN_VERIFY=1: read the scattered arrays back and
       * check every deposit sits inside the cell range the gather probes for
       * its position (hash(pos[i]) == cell of slot i). Any misplaced entry is
       * invisible to the gather - the device-binning energy-loss suspect. */
      static const bool bin_verify = getenv("CYCLESPLUS_PHOTON_BIN_VERIFY") != nullptr;
      if (bin_verify) {
        dscene->photon_pos.copy_from_device();
        const float4 *pos = dscene->photon_pos.data();
        const int *cs = grid->cell_start;
        uint64_t misplaced = 0, zero_w = 0;
        int cell = 0;
        for (int i = 0; i < num; i++) {
          while (cell < table && cs[cell + 1] <= i) {
            cell++;
          }
          const float4 p = pos[i];
          const uint b = photon_grid_hash((int)floorf(p.x * grid->inv_cell),
                                          (int)floorf(p.y * grid->inv_cell),
                                          (int)floorf(p.z * grid->inv_cell),
                                          (uint)table);
          if ((int)b != cell) {
            misplaced++;
          }
        ...
        LOG_INFO << "CyclesPlus BIN_VERIFY scatter: num " << num << " misplaced " << misplaced
                 << " zero-normal " << zero_w;
      }
    }
    else {
      /* Scatter failed: publish an empty grid rather than binding zeroed
       * deposits (the gather would count a zero-contribution generation). */
      LOG_WARNING << "CyclesPlus photon map: device scatter failed, dropping generation";
      kintegrator->photon_num = 0;
      kintegrator->photon_table_size = 0;
      kintegrator->photon_radius = 0.0f;
      kintegrator->photon_inv_cell = 0.0f;
  ...
  photon_gather_pending_ = (generation != 0);
...
void PathTrace::photon_gather_after_work()
{
  ...
  /* No photon grid uploaded (yet): the gather would be a whole-frame no-op
