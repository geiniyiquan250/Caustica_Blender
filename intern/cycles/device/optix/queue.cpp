...
#  endif
   ...
   * queue - staging into the shared buffer live-clobbered a running render
   * kernel's parameters (it then read the photon deposit pointer as its
   * path index array: wild writes, dead driver; only OptiX died because
   * CUDA passes per-launch value arguments). The private buffer first
   * snapshots the shared one (device-to-device on this stream, so it also
   * carries current kernel data), then takes the photon arguments - the two
   * launch streams can no longer touch each other's parameters. */
  const bool photon_kernel = (kernel == DEVICE_KERNEL_PHOTON_TRACE);
