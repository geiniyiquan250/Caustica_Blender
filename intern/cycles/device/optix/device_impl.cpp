...
#  endif
  ...
  { /* Create intersection-only pipeline. */
     ...
     * generation stack sizes. (The photon raygen lives in its own pipeline
     * below and no longer contributes here.) */
  ...
  { /* CyclesPlus: dedicated photon pipeline - see the PIP_PHOTON note in
     ...
     * render launches never share pipeline-internal state (the documented
     * OptiX constraint). NOTE for anyone bisecting with
     * get_pipeline_stack_size: it fills the vector by GLOBAL program group
     * enum, NOT by pipeline position - the photon raygen must be looked up
     * as PG_RGEN_PHOTON_TRACE (a position index once read a foreign zero
     * entry and the accurate-mode raygen overran the stack). */
    vector<OptixProgramGroup> pipeline_groups;
    pipeline_groups.reserve(NUM_PROGRAM_GROUPS);
    pipeline_groups.push_back(groups[PG_RGEN_PHOTON_TRACE]);
    pipeline_groups.push_back(groups[PG_MISS]);
    pipeline_groups.push_back(groups[PG_HITD]);
    pipeline_groups.push_back(groups[PG_HITS]);
    pipeline_groups.push_back(groups[PG_HITL]);
    ...
    optix_assert(optixPipelineCreate(context,
                                     ...
                                     nullptr,
                                     nullptr,
                                     &pipelines[PIP_PHOTON]));

    unsigned int trace_css;
    vector<OptixStackSizes> stack_size = get_pipeline_stack_size(
        pipelines[PIP_PHOTON], pipeline_groups, trace_css);
    const unsigned int css = stack_size[PG_RGEN_PHOTON_TRACE].cssRG +
                             link_options.maxTraceDepth * trace_css;
    LOG_INFO << "CyclesPlus: photon pipeline css " << css << " (photon raygen "
             << stack_size[PG_RGEN_PHOTON_TRACE].cssRG << ")";
