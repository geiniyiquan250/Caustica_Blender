                                  ...
                                  const DeviceInfo &denoise_device_info)
{
   ...
   * on one card and the finished map is replicated to all of them (see
   * photon_pick_trace_device). Before that, ticking the checkbox on such a
   * rig killed the process outright - the tracer asked the MultiDevice
   * wrapper for a queue and the base class answers that with a LOG_FATAL
   * (user report 2026-08-18, 4-5 cards).
   *
   * The distributed path could not be tested on real multi-GPU hardware
   * here, so CYCLESPLUS_PHOTON_SINGLE_GPU=1 restores the old behaviour: the
   * solver switches off above one device and the render is plain Blender.
   * Turning it off HERE rather than inside PhotonMap is what makes that
   * true - use_photon_caustics also drives the path-traced caustics
   * suppression and the scrambling-distance override below, so gating it
   * deeper would leave the render darker than stock instead of equal to it.
   *
   * multi_devices is only populated for a real multi device, so a single
   * card never sees this (validated: wegb control 0.015612 bit-exact). */
  static const bool photon_single_gpu = getenv("CYCLESPLUS_PHOTON_SINGLE_GPU") != nullptr;
  const bool photon_multi_device = scene->device != nullptr &&
                                   scene->device->info.multi_devices.size() > 1;
  const bool photon_requested = get_boolean(cscene, "use_photon_caustics");
  if (photon_requested && photon_multi_device && photon_single_gpu) {
    LOG_WARNING << "CyclesPlus: photon caustics disabled by CYCLESPLUS_PHOTON_SINGLE_GPU - "
                << scene->device->info.multi_devices.size()
                << " render devices are selected. Enable exactly one GPU under "
                   "Preferences > System to use the solver.";
  }
   ...
   * estimates congeal into equal-sized screen-aligned blocks ("giant
   * pixels", user car scenes; confirmed: scrambling off dissolves them).
   * Photon renders force full decorrelation - silent, per the one-click
   * product rule. */
  if (use_photons && scrambling_distance != 1.0f) {
    LOG_INFO << "CyclesPlus: scrambling distance disabled (correlated tiles "
                                     ...
                                     blender::ViewLayer &b_view_layer)
{
   ...
   * repeating that condition here - keeps the two sides from ever drifting
   * apart, which would either lose a pass or log it as unknown below. */
  unordered_set<string> declared_passes;
  for (blender::RenderPass &b_pass : b_rlay.passes) {
    declared_passes.insert(b_pass.name);
  }
  ...
  for (blender::ViewLayerLightgroup &b_lightgroup : b_view_layer.lightgroups) {
    ...
    /* The same split for the caustic. Noisy on purpose: Cycles does not
     * denoise its own light group passes either, and a denoised group pass
     * would need the noisy twin that made the plain Caustics pass read black
     * until that trap was understood. It also keeps the sum of the groups
     * exactly equal to the single Caustics pass, which is the test this
     * feature stands or falls on - denoising is non-linear and would break
     * that identity. */
    const string caustics_name = string_printf("Caustics_%s", b_lightgroup.name);
    if (declared_passes.contains(caustics_name)) {
      Pass *caustics_pass = pass_add(scene, PASS_CAUSTICS, caustics_name.c_str(),
                                     PassMode::NOISY);
      caustics_pass->set_lightgroup(ustring(b_lightgroup.name));
