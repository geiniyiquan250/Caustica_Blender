...
enum_denoising_upscale_quality = (
    ...
    ('BALANCED', "Balanced", "Offers both optimized performance and image quality", 2),
...
class CyclesRenderSettings(bpy.types.PropertyGroup):
    ...
    photon_caustics_casters: EnumProperty(
        ...
        items=(
            ('ALL', "All Materials", "Every material that would cast caustics in real "
             "life casts them (physically complete, default)"),
            ('SELECTED', "Selected Materials Only", "Only materials with \"Cast Photon "
             "Caustics\" enabled in their settings cast photon caustics; all other "
             "materials keep Cycles' regular behavior, window glass included. "
             "Concentrates the photon budget on the materials that matter "
             "(interiors: leave the windows unflagged)"),
        ),
        default='ALL',
    )
...
class CyclesMaterialSettings(bpy.types.PropertyGroup):
    ...
    photon_cast: BoolProperty(
        ...
        "mode (Render Properties > Light Paths > Caustics) is set to Selected "
        "Materials Only. Unflagged materials keep Cycles' regular caustics. "
...
class CyclesLightSettings(bpy.types.PropertyGroup):
    ...
    photon_cast: BoolProperty(
        ...
        "fill lights that should not produce caustics: their share of the photon "
        "budget goes to the remaining lights, so the caustics that matter resolve "
...
class CyclesWorldSettings(bpy.types.PropertyGroup):
    ...
    photon_cast: BoolProperty(
        ...
        "(sun discs and bright regions of an HDRI). Turn it off to keep the "
...
class CyclesRenderLayerSettings(bpy.types.PropertyGroup):
    ...
    use_pass_caustics: BoolProperty(
        ...
        "separately in compositing. Subtract it from Combined to get the render "
        "without caustics. Requires Photon Caustics to be enabled",
        default=False,
        update=update_render_passes,
    )
...
class CyclesPreferences(bpy.types.AddonPreferences):
    ...
    def has_dlss_gpu_devices(self):
        ...
        if compute_device_type == 'NONE':
            return False
        for device in self.get_device_list(compute_device_type):
            if device[1] != 'CUDA' or len(device) <= 9:
                continue
