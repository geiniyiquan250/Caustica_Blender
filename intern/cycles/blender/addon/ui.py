...
class CYCLES_RENDER_PT_light_paths_caustics(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        # (photon_pick_trace_device in photon_map.cpp), so the panel says the
        # same thing on one card and on five.

        # Material report ("fail loudly"): materials whose fast-mode photon
        # profile is approximate or silently caustic-less. Filled by the last
        # render/viewport session; empty means every material was exact.
        if cscene.use_photon_caustics:
            import _cycles
            report = _cycles.photon_material_report()
            if report:
                box = layout.box()
...
class CYCLES_RENDER_PT_passes_light(CyclesButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        # Only meaningful with the photon solver on - the caustics never reach
        # the light passes, so this is the only way to isolate them.
        sub = col.column(align=True)
