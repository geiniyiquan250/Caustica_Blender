...
static PyObject *photon_material_report_func(PyObject * /*self*/, PyObject * /*args*/)
{
  const vector<PhotonMaterialReport> report = photon_material_report_snapshot();
  PyObject *ret = PyTuple_New(report.size());

  for (size_t i = 0; i < report.size(); i++) {
    PyObject *entry = PyTuple_New(3);
    PyTuple_SET_ITEM(entry, 0, pyunicode_from_string(report[i].name.c_str()));
    PyTuple_SET_ITEM(entry, 1, PyLong_FromLong(report[i].level));
    PyTuple_SET_ITEM(entry, 2, pyunicode_from_string(report[i].reason.c_str()));
    PyTuple_SET_ITEM(ret, i, entry);
  }
