 ...
 * whether a material can throw caustics at all - a wood table carrying a
 * black metallic map cannot, brass carrying a white one can. It used to read
 * the file from disk with OIIO, which works only as long as the file IS on
 * disk. Most real scenes ship their textures PACKED inside the .blend (asset
 * packs, files passed to a client), and then Cycles never sets a filename at
 * all: it reads the pixels straight from the datablock. With no verdict the
 * classifier had to assume the material might cast, so tables, tiles and
 * floors started throwing photons - the budget spread across dozens of
 * surfaces that focus nothing, and the caustics broke up into the separate
 * gather discs (user report: "All Materials" turning a drinks scene into
 * white blobs).
 *
 * Sampling here costs one buffer the renderer is about to load anyway, on a
 * sparse grid of at most ~4k texels. Cached per image, since one texture
 * usually feeds several nodes. */
static float photon_image_texture_mean(blender::Image *b_image,
                                       blender::ImageUser *b_image_user)
{
  static thread_mutex mean_mutex;
  static unordered_map<const void *, float> mean_cache;
  {
    const thread_scoped_lock lock(mean_mutex);
    const auto it = mean_cache.find((const void *)b_image);
  ...
  blender::ImBuf *ibuf = BKE_image_acquire_ibuf(b_image, b_image_user, &lock_handle);
  if (ibuf != nullptr && ibuf->x > 0 && ibuf->y > 0) {
    const int channels = std::max(ibuf->channels, 1);
    const int step_x = std::max(ibuf->x / 64, 1);
    const int step_y = std::max(ibuf->y / 64, 1);
    const float *fbuf = ibuf->float_data();
    const uint8_t *bbuf = (const uint8_t *)ibuf->byte_buffer.data;
    double sum = 0.0;
    size_t n = 0;
    for (int y = 0; y < ibuf->y; y += step_y) {
      for (int x = 0; x < ibuf->x; x += step_x) {
        const size_t idx = ((size_t)y * ibuf->x + x);
        if (fbuf != nullptr) {
          sum += fbuf[idx * channels];
        }
        else if (bbuf != nullptr) {
          /* Non-colour data (metallic, roughness, masks) is stored raw, which
           * is exactly what the classifier wants. */
          sum += bbuf[idx * 4] * (1.0 / 255.0);
        }
        else {
          continue;
        }
  ...
  BKE_image_release_ibuf(b_image, ibuf, lock_handle);

  {
    const thread_scoped_lock lock(mean_mutex);
    mean_cache[(const void *)b_image] = mean;
  }
  return mean;
}
                            ...
                            blender::bNode &b_node)
{
  ...
  if (b_node.is_type("ShaderNodeVectorCurve"_ustr)) {
    ...
    if (b_image) {
      ...
      /* CyclesPlus: works for packed images too, unlike reading the file.
       * Only with photon caustics switched on - with the box unticked this
